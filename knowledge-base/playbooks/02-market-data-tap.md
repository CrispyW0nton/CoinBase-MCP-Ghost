...
5. `coinbase_record { durationMs, sampleIntervalMs }` — long OBSERVE recording
   when WS TAP VIABLE is NO. It samples DOM order book/trades tape only, writes
   fully-provenanced `source:"dom"` events, quarantines malformed provenance,
   and emits a manifest with health/disconnect/provenance counts. No clicks,
   REST, SDK, or sockets.
