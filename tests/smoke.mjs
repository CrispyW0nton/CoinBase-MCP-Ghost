...
function stage1ApprovedCredentialEnv() {
  return {
...
async function offlineSuite() {
  ...
  await check("Stage 1 feed preflight combines approved credentials and subscription plan", () => {
    const env = stage1ApprovedCredentialEnv();
  ...
  await check("Stage 1 readiness rejects live-flagged manifests without preflight evidence", async () => {
    const now = new Date().toISOString();
    const baseDir = fs.mkdtempSync(path.join(os.tmpdir(), "cmcp-stage1-preflight-evidence-"));
    async function* frameSource() {
      yield stage1Heartbeat(74, 1, now);
      yield {
        channel: "level2",
        sequence_num: 75,
        timestamp: now,
    ...
    await recordStage1FrameSource({
      ...
      manifestMeta: {
        ...
        keyedClientImplemented: true,
        liveWsFlowObserved: true,
        evidence: { testOnly: true, reason: "missing preflight evidence" }
      }
    });
    const readiness = await stage1Readiness({ journalDir, recordingsDir, horizonObservations: 1 });
    assert.equal(readiness.liveEvidenceGate.pass, false);
  ...
  await check("Stage 1 readiness rejects live-flagged manifests with missing journal evidence", async () => {
    const now = new Date().toISOString();
    const baseDir = fs.mkdtempSync(path.join(os.tmpdir(), "cmcp-stage1-journal-missing-"));
    async function* frameSource() {
      yield stage1Heartbeat(75, 1, now);
      yield {
        channel: "level2",
        sequence_num: 76,
        timestamp: now,
    ...
    const recorded = await recordStage1FrameSource({
      ...
      manifestMeta: {
        ...
        keyedClientImplemented: true,
        liveWsFlowObserved: true,
        evidence: { preflight, reason: "simulated missing journal evidence" }
      }
    });
    const manifest = JSON.parse(fs.readFileSync(recorded.manifestPath, "utf8"));
    manifest.journalPath = path.join(baseDir, "journal", "BTC-USD", "missing.jsonl");
  ...
  await check("Stage 1 readiness rejects live-flagged manifests with postdated preflight evidence", async () => {
    const now = new Date().toISOString();
    const baseDir = fs.mkdtempSync(path.join(os.tmpdir(), "cmcp-stage1-preflight-postdated-"));
    async function* frameSource() {
      yield stage1Heartbeat(78, 1, now);
      yield {
        channel: "level2",
        sequence_num: 79,
        timestamp: now,
    ...
    const recorded = await recordStage1FrameSource({
      ...
      manifestMeta: {
        ...
        keyedClientImplemented: true,
        liveWsFlowObserved: true,
        evidence: { preflight, testOnly: true, reason: "simulated postdated preflight" }
      }
    });
    const manifest = JSON.parse(fs.readFileSync(recorded.manifestPath, "utf8"));
    manifest.evidence.preflight.generatedAt = new Date(Date.parse(manifest.startedAt) + 1000).toISOString();
    fs.writeFileSync(recorded.manifestPath, JSON.stringify(manifest, null, 2), "utf8");

    const readiness = await stage1Readiness({ journalDir, recordingsDir, horizonObservations: 1 });
    assert.equal(readiness.liveEvidenceGate.pass, false);
    assert.match(readiness.liveEvidenceGate.reasons.join("; "), /preflight generatedAt is after manifest startedAt/);
  });
  ...
  await check("Stage 1 readiness rejects live-flagged manifests without frame evidence", async () => {
    ...
    async function* frameSource() {
      yield stage1Heartbeat(79, 1, now);
      yield {
        channel: "level2",
        sequence_num: 80,
        timestamp: now,
    ...
    const recorded = await recordStage1FrameSource({
      ...
      manifestMeta: {
        ...
        keyedClientImplemented: true,
        liveWsFlowObserved: true,
        evidence: { preflight, testOnly: true, reason: "simulated malformed manifest" }
      }
    });
    const manifest = JSON.parse(fs.readFileSync(recorded.manifestPath, "utf8"));
    delete manifest.frameEvidence;
    fs.writeFileSync(recorded.manifestPath, JSON.stringify(manifest, null, 2), "utf8");
